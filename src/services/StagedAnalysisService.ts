...
export class StagedAnalysisService {
  ...
  /**
   * Process jobs that are in 'not-started' status
  ...
  /**
   * Process jobs that are in 'prompt-forming' status
  ...
  /**
   * Process jobs that are in 'model-analysis' status
   * This runs the AI analysis and completes the job
   */
  static async processModelAnalysisJobs(): Promise<void> {
    ...
    if (!job) {
      console.debug('📋 No model-analysis jobs found to process');
      return;
    }
    ...
    try {
      ...
      console.error(`❌ Error running analysis for job ${job.id}:`, error);
